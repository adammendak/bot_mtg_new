"""OHLC loaders: local cache, optional Capital DEMO mids, HistData M1, Dukascopy, Coinbase.

Capital DEMO credentials (``CAPITAL_API_KEY`` / ``CAPITAL_EMAIL`` /
``CAPITAL_API_PASSWORD``) are preferred when present. Research falls back to
real HistData M1 (resampled to M5) for XAU / US100=NSXUSD / US500=SPXUSD,
Dukascopy M1 (resampled to M5) for US30=USA30.IDX/USD, and Coinbase Exchange
5m for BTC — never invented prices.
"""
...
# HistData has NSX/USD (Nasdaq 100) and SPX/USD (S&P 500). It does **not**
# publish DJIA / US30 (UDX/USD is the Dollar Index, not Dow).
HISTDATA_PAIRS = {
    "XAU": "XAUUSD",
    "US100": "NSXUSD",
    "US500": "SPXUSD",
...
# Dukascopy cash-index CFDs (UTC stamps). Used when HistData has no pair.
# USA30.IDX/USD prints at DJIA cash (~52k in Sep 2026), not YM futures.
DUKASCOPY_INSTRUMENTS = {
    "US30": "USA30.IDX/USD",
    "US500": "USA500.IDX/USD",
    "US100": "USA100.IDX/USD",
    "GER40": "DEU.IDX/EUR",
    "J225": "JPN225.IDX/JPY",
    "XAU": "XAU/USD",
    "XAG": "XAG/USD",
...
SERIES_NOTES = {
    "XAU": "HistData XAUUSD (spot gold). Same series as PR #139/#143.",
    "US100": "HistData NSXUSD = Nasdaq 100 cash (NQ / US100). Capital epic US100. Same series as PR #139/#143.",
    "US500": "HistData SPXUSD = S&P 500 cash (ES / US500). Capital epic US500. Analogous vendor/method to US100.",
    "US30": "HistData has no DJIA pair. Dukascopy USA30.IDX/USD = Dow Jones cash (YM / US30). Capital epic US30. M1→M5, UTC native (not Eastern).",
    "GER40": "HistData GRXEUR = DAX cash (DE40 / GER40). Dukascopy DEU.IDX/EUR fallback.",
    "EURUSD": "HistData EURUSD spot. Same vendor as prior FX bakeoffs.",
    "BTC": "Coinbase Exchange BTC-USD 5m public candles. HistData has no BTC.",
    "BTCUSD": "Coinbase Exchange BTC-USD 5m public candles. HistData has no BTC.",
    "XAG": "HistData XAGUSD (spot silver). Dukascopy XAG/USD fallback.",
    "J225": "HistData JPXJPY (Nikkei 225 cash) if the pair exists; else Dukascopy JPN225.IDX/JPY.",
    "USDJPY": "HistData USDJPY spot. Dukascopy USD/JPY fallback.",
}

PRIMARY_SYMBOLS = ["XAU", "BTC", "US100", "US500", "US30", "GER40", "EURUSD"]
EXTRA_SYMBOLS = ["XAG", "J225", "USDJPY"]
SYMBOLS = PRIMARY_SYMBOLS + EXTRA_SYMBOLS
...
def fetch_dukascopy_m1(symbol: str, start: datetime, end: datetime) -> pd.DataFrame:
    """Dukascopy cash-index M1 (UTC). Fallback when HistData has no pair."""
    instrument = DUKASCOPY_INSTRUMENTS[symbol]
    slug = instrument.replace("/", "").replace(".", "").replace("&", "")
    path = _ensure_cache() / f"{symbol}_{slug}_M1.csv"
    if path.exists() and path.stat().st_size > 1000:
    ...
    if raw is None or raw.empty:
        raise RuntimeError(f"Dukascopy returned no M1 bars for {instrument}")
    out = raw[["open", "high", "low", "close"]].astype(float).copy()
...
def series_note(symbol: str) -> str:
    return SERIES_NOTES.get(symbol, "")
...
def load_symbol(symbol: str, start: datetime, end: datetime, prefer_capital: bool = True) -> Bars:
    ...
    if m5 is None and symbol in HISTDATA_PAIRS:
        try:
            ...
            gaps.append(
                f"HistData {HISTDATA_PAIRS[symbol]} M1 (Eastern stamps → UTC), resampled to M5. "
                f"Not Capital mids. {series_note(symbol)}"
            )
        except Exception as e:
            gaps.append(f"histdata_unavailable {HISTDATA_PAIRS[symbol]}: {e}")

    if m5 is None and symbol in DUKASCOPY_INSTRUMENTS:
        try:
            m1 = fetch_dukascopy_m1(symbol, start, end)
            n_m1 = len(m1)
            m5 = resample_ohlc(m1, 5)
            source = "dukascopy_m1_resampled_m5"
            gaps.append(
                f"Dukascopy {DUKASCOPY_INSTRUMENTS[symbol]} M1 (UTC bid), resampled to M5. "
                f"Not Capital mids. {series_note(symbol)}"
            )
        except Exception as e:
            gaps.append(f"dukascopy_unavailable {DUKASCOPY_INSTRUMENTS[symbol]}: {e}")

    if m5 is None and symbol in ("BTCUSD", "BTC"):
