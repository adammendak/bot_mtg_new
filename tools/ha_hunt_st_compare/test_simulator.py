...
class FixedRrAndEntryModeTests(unittest.TestCase):
    def _fixed(self, **kw):
        from tools.ha_hunt_st_compare.simulator import st_v3_fixed_rr

        return st_v3_fixed_rr("band_st", 1.0, 5, 45, **kw)

    def test_factory_locks_st_7_2_flat_fixed(self):
        from tools.ha_hunt_st_compare.simulator import st_v3_fixed_rr, st_v3_prod_like

        p = st_v3_fixed_rr("band_st", 1.5, 15, 60)
        self.assertEqual(p.st_atr_len, 7)
        self.assertEqual(p.st_factor, 2.0)
        self.assertTrue(p.fixed_rr)
        self.assertFalse(p.scale_tp1)
        self.assertFalse(p.allow_pyramid)
        self.assertFalse(p.exit_band_cross)
        self.assertEqual(p.tp1_mult, 1.5)
        self.assertEqual(p.m45_minutes, 60)
        self.assertEqual(p.entry_mode, "band_st")
        self.assertTrue(p.st_stop_strict)

        prod = st_v3_prod_like(15, 60)
        self.assertTrue(prod.scale_tp1)
        self.assertEqual(prod.tp1_mult, 2.0)
        self.assertTrue(prod.exit_band_cross)
        self.assertFalse(prod.exit_ha_flip)
        self.assertFalse(prod.fixed_rr)
        self.assertEqual(prod.st_atr_len, 7)

    def test_fixed_rr_closes_at_plus_or_minus_one(self):
        book = simulate(
            "SYN",
            _synthetic_trend(1600),
            self._fixed(),
        )
        self.assertGreaterEqual(book.n, 0)
        for t in book.trades:
            self.assertNotIn(t.reason, {"m45_st_flip", "h1_st_flip", "m45_slow_band", "trail", "band_cross"})
            if t.reason == "stop":
                self.assertAlmostEqual(t.r, -1.0, places=5)
            if t.reason.startswith("tp_"):
                self.assertAlmostEqual(t.r, 1.0, places=5)

    def test_st_flip_and_h4_params_run(self):
        from tools.ha_hunt_st_compare.simulator import st_v3_fixed_rr

        m5 = _synthetic_trend(2400)
        flip = simulate("SYN", m5, st_v3_fixed_rr("st_flip", 1.0, 5, 45))
        self.assertGreaterEqual(flip.n, 0)
        h1 = resample_bars(m5, 60)
        h4 = simulate("SYN", h1, st_v3_fixed_rr("band_st", 1.0, 60, 240))
        self.assertGreaterEqual(h4.n, 0)

    def test_window_excludes_early_entries(self):
        from tools.ha_hunt_st_compare.simulator import st_v3_fixed_rr

        m5 = _synthetic_trend(1600)
        full = simulate("SYN", m5, st_v3_fixed_rr("band_st", 1.0, 5, 45))
        late = simulate(
            "SYN",
            m5,
            st_v3_fixed_rr(
                ...
                5,
                45,
                trade_start="2025-09-03T00:00:00",
                trade_end="2025-09-04T00:00:00",
            ),
        )
        self.assertGreaterEqual(full.n, late.n)
        for t in late.trades:
            self.assertGreaterEqual(t.entry_time, "2025-09-03T00:00:00")
            self.assertLess(t.entry_time, "2025-09-04T00:00:00")

    def test_band_only_strict_stop_skips_wrong_side(self):
        from tools.ha_hunt_st_compare.simulator import st_v3_fixed_rr

        m5 = _synthetic_trend(1600)
        a = simulate("SYN", m5, st_v3_fixed_rr("band_st", 1.0, 5, 45))
        c = simulate("SYN", m5, st_v3_fixed_rr("band_only", 1.0, 5, 45))
        # Wrong-side ST stops are rejected, so C cannot invent extra valid tickets
        # beyond A's band-cross+ST set on this path (may be equal or close).
        self.assertGreaterEqual(c.n, 0)
        self.assertLessEqual(abs(c.n - a.n), max(a.n, 1))
