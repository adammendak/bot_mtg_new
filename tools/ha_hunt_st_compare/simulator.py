...
from .indicators import atr, heikin_ashi, rma, rma_band, supertrend, tqi_series
...
class Params:
    ...
    # ST_V3 last-quarter bakeoff (defaults preserve the older HA-Hunt cells).
    entry_mode: str = "band_st"  # band_st | st_flip | band_only
    fixed_rr: bool = False  # True = full close at +tp1_mult R (no half / BE / runner)
    exit_band_cross: bool = False  # prod runner: LTF fast band vs opposite slow
    st_stop_strict: bool = False  # skip entry when ST line is not protective
    trade_start: Optional[str] = None  # ISO UTC; entries only on/after this bar
    trade_end: Optional[str] = None  # ISO UTC; entries only before this bar
    # SATS-idea A/B on ST_V3 (defaults preserve prod-like).
    tqi_min: Optional[float] = None  # skip band-cross unless chart-TF TQI ≥ this
    exit_char_flip: bool = False  # TQI collapse (window) flattens remaining
...
def simulate(symbol: str, bars: Bars, p: Params) -> Book:
    ...
    # Bias + default SL Supertrend: same series as the structure HTF, or a
    # separately aggregated TF (H1=60, H4=240, …).
    if p.st_tf_minutes == p.m45_minutes:
        st_line, st_dir_arr = m45_st, m45_dir
        st_closed = closed_i
    else:
        sts, _, sth, stl, stc = _aggregate_htf(_bucket_starts(bars.time, p.st_tf_minutes), o, h, l, c)
        if len(stc) < p.st_atr_len + p.htf_closed_shift + 5:
            return _metrics([], symbol, p.name)
        st_line, st_dir_arr = supertrend(sth, stl, stc, p.st_factor, p.st_atr_len)
        st_closed = _map_closed_htf(chart_starts, sts, p.st_tf_minutes, p.htf_closed_shift, chart_min)
    ...
    flip_reason = {
        ...
        60: "h1_st_flip",
        240: "h4_st_flip",
    }.get(p.st_tf_minutes, f"st{p.st_tf_minutes}_flip")
    ...
    tqi_chart = None
    tqi_win_high = None
    if p.tqi_min is not None or p.exit_char_flip:
        chart_atr = atr(h, l, c, 14)
        tqi_chart, _, _ = tqi_series(h, l, c, chart_atr)
        tqi_win_high = np.full(n, np.nan, dtype=np.float64)
        w = max(int(p.char_flip_window), 1)
        for k in range(w, n):
            tqi_win_high[k] = float(np.nanmax(tqi_chart[k - w : k]))
    ...
    for i in range(n):
        ...
        if pos != 0:
            ...
            if exit_stop:
                ...
                if p.use_tp1 and not tp1_hit and ((pos == 1 and h[i] >= tgt) or (pos == -1 and l[i] <= tgt)):
                    if p.fixed_rr:
                        close_trade(i, float(tgt), f"tp_{p.tp1_mult:g}r")
                        # stop already evaluated above; full close at exact +N R
                    else:
                        tp1_hit = True
                ...
                # SATS-style TQI collapse as an early flatten (before or after TP1).
                if pos != 0 and p.exit_char_flip and tqi_chart is not None and st_age >= p.char_flip_min_age:
                    qi = tqi_chart[i]
                    qh = tqi_win_high[i]
                    collapsed = (not np.isnan(qh) and not np.isnan(qi) and qh > p.char_flip_high and qi < p.char_flip_low)
                    if collapsed:
                        close_trade(i, float(c[i]), "char_flip")
                ...
                # Prod ST_V3 runner (PR #150): after TP1, flatten when the
                # entry-TF fast RMA band crosses to the opposite side of slow.
                if pos != 0 and p.exit_band_cross and tp1_hit:
                    if pos == 1:
                        exit_bx = (not np.isnan(f_up[i]) and not np.isnan(s_up[i]) and f_up[i] < s_up[i])
                    else:
                        exit_bx = (not np.isnan(f_lo[i]) and not np.isnan(s_lo[i]) and f_lo[i] > s_lo[i])
                    if exit_bx:
                        close_trade(i, float(c[i]), "band_cross")
        ...
        ready = fills < p.cap_reg and i >= warmup and j >= 0 and st_closed[i] >= 0 and in_window
        if p.allow_pyramid:
        ...
        if p.entry_mode == "st_flip":
            long_raw = st_flip and st_bull
            short_raw = st_flip and st_bear
        elif p.entry_mode == "band_only":
            long_raw = bool(cross_long[i]) and gate_l
            short_raw = bool(cross_short[i]) and gate_s
        else:
            long_raw = st_bull and bool(cross_long[i]) and gate_l
            short_raw = st_bear and bool(cross_short[i]) and gate_s
        if p.tqi_min is not None and tqi_chart is not None:
            qi = tqi_chart[i]
            tqi_ok = (not np.isnan(qi)) and qi >= p.tqi_min
            long_raw = long_raw and tqi_ok
            short_raw = short_raw and tqi_ok
        ...
        if long_sig or short_sig:
            ...
            if is_add:
                ...
                if p.st_stop_strict:
                    sl = st_val(st_line, i)
                    valid_sl = (not np.isnan(sl)) and ((sl < c[i]) if is_long else (sl > c[i]))
                    if not valid_sl:
                        sl = np.nan
                else:
                    sl = stop_price(i, is_long)
                r_dist = abs(c[i] - sl) if not np.isnan(sl) else 0.0
...
def _st_v3_locked(**overrides) -> Params:
    """Current ST_V3 Java lock: ATR 7 / factor 2.0, flat, HTF structure, ST stop.

    Structure HTF is the same series as the Supertrend TF (StV3Engine), not
    the older pine M45-always gate. Defaults here are the prod-like runner
    (half @ 1:2 → BE → entry-TF band-cross). Fixed-RR cells override exit flags.
    """
    base = dict(
        slow_len=144,
        fast_len=33,
...
def st_v3_fixed_rr(
    ...
    """Full close at +rr R; SL stays at the entry Supertrend line."""
    mode_tag = {"band_st": "A", "st_flip": "B", "band_only": "C"}.get(entry_mode, entry_mode)
    return _st_v3_locked(
        name=f"STV3_{mode_tag}_rr{rr:g}_m{chart_minutes}_st{st_tf_minutes}",
        entry_mode=entry_mode,
        fixed_rr=True,
        scale_tp1=False,
        tp1_mult=rr,
        exit_band_cross=False,
        exit_ha_flip=False,
        req_m45_struct=entry_mode != "st_flip",
        chart_minutes=chart_minutes,
        st_tf_minutes=st_tf_minutes,
        m45_minutes=st_tf_minutes,
...
def st_v3_prod_like(
    ...
    trade_start: Optional[str] = None,
    trade_end: Optional[str] = None,
    tqi_min: Optional[float] = None,
    exit_char_flip: bool = False,
    exit_band_cross: bool = True,
    name: Optional[str] = None,
) -> Params:
    """Prod-style half@1:2 + BE + band-cross runner (PR #150)."""
    tag = f"STV3_prod_m{chart_minutes}_st{st_tf_minutes}"
    if tqi_min is not None:
        tag += f"_tqi{tqi_min:g}"
    if exit_char_flip and exit_band_cross:
        tag += "_cfplus"
    elif exit_char_flip:
        tag += "_cfonly"
    return _st_v3_locked(
        ...
        trade_end=trade_end,
        tqi_min=tqi_min,
        exit_char_flip=exit_char_flip,
        exit_band_cross=exit_band_cross,
    )
